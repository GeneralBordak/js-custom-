...
Скрытие первой серии ( и из легенды) для показа.

1.2. Толщина линии в графике (код JS) и маркер
...
Изменить значение «4» на нужную толщину. w.series[0] – номер серии. Необходимо для всех серий прописать.

1.3. Убрать белую обводку вокруг столбца или сектора (код JS)
Код пишется до стандартного набора «Highcharts.chart({..})». В гистограмме вокруг столбца:

w.plotOptions.series.borderColor = 'transparent';
На круговой диаграмме вокруг сектора:

w.plotOptions.pie.borderColor = 'transparent';
1.4. Изменить местами серии на гистограмме (код JS)
Код пишется до стандартного набора «Highcharts.chart({..})».

w.series = w.series.reverse();
1.5. Возможность изменять местами категории на гистограмме (код JS)
w.xAxis.categories = w.xAxis.categories.reverse();
1.6. Вывести нужные столбцы или поменять местами столбцы (код JS)
w.series = [ w.series[2] ,w.series[4],w.series[3]];
В связи с первым появлением и второй серией не будут, 4 и 5 серии поменялись местами (нумерация с 0).

1.7. Скрытие треугольника большого числа значений на виджете (код JS)
$('[id*="va-widget-warning"]').css({'display':'none'});
1.8. Показать серии измерений в гистограмме
w.series[0].data = w.series[0].data.slice(0,10);
Последнее числовое значение показывает количество значений отобразить.

1.9. Убрать обводку подписки на серийный текст
w.plotOptions.series.dataLabels.style.textOutline = false;
1.10. Убрать тень (подсветку) подписки на данные
w.plotOptions.series.dataLabels.style.textOutline = false; 
1.11 Развернуть легенду. Для гистограммы с накоплением
w.legend.reversed = true;
1.12. Отсортировать круговую диаграмму от большего к меньшему, округление до 2 символов (код JS)
...
*.toFixed(2)-обрезание значения до 2-х знаков после запятой. В скобках количество знаков после запятой. Важно: значение просто обрезается, а не округляется!
...
1.13 Покрасить отрицательные значения серии в другом цвете
w.series[0].negativeColor = 'red'; //Красим точки первой серии красным если они меньше 0
1.14 Размер баров
Если бары не помещаются в пределах границ виджета, или самый длинный бар очень длинный, что его присутствие проявляется на телебаре, встречается с определенной строчкой, меняя то, что после знака =. Вставить в начало кода.
...
Этот код применяет настройки маркеров ко всем сериям.
...
Если необходимо, можно применить свойства к одной конкретной серии. Вот так:

 w.series[0].marker.symbol = 'circle' // сделать маркеры первой кругами
Если необходимо, можно применить свойства к одной конкретной серии. Вот так w.series[номер серии].marker.symbol = 'circle' - сделать маркеры кругами
...
Вставляется в самое начало: удобно, когда название меняется, и чтобы не прокликивать все в PG, можно поменять так /
...
1.23 Положение серии выше
Вставляется в самое начало: чем выше цифры, тем выше серия /

    w.series[i].zIndex = 99;
1.24 Скрыть подсказку
Вставляется вместо всплывающей подсказки: w.tooltip /

        tooltip: {
            backgroundColor:'white',
            enabled: false
        }
1.25 Изменить отображение данных
Для работы необходимо положить на сервер трафика: ..., расположенную по адресу: \serv2\opr$\Библиотека аналитика\Акту виджеты\
///d - дата для конывертации ///clv - календарный разрез: /// 0 (undefined) - день, месяц и год /// 1-год /// 2-полугодие /// 3-квартал /// 4-месяц /// 5-неделя /// 6- /// 7-день

функция getDateString(d, clv){
...
1.27 Заменить заголовок виджета автоматически по наименованию индекса серии (заголовок должен быть включен для отображения в начале)
...
2 Форматирование подписки
2.1 Использование форматирования через поле в сетке свойств DD
В поле «Текст» возможны переменные для форматирования:
...
Задать запись с воспроизведением текста в центре круговой
...
Форматирование подписи круговой диаграммы с указанием наименования серий и наименований точек. Может быть взято в порядке следствия "Источник информации", а в строке измерения "Категория информации". В случае серии.название отрезается часть наименования, включающее в себя название признака, пришедшего из DC. Без использования функции replace подсказка выглядит: "__Обращения граждан - Портал заражений - Благоустройство: 5", с использованием функции replace: "Портал зависимий -устройство: 5"
...
Округлит до двух знаков после запятой и покрасит подпись в столбце

Добавить в фильтр с измерением "Номер недели", надпись "NN неделя общественного года"
@value + " неделя " + (new Date().getFullYear())
new Date().getFullYear() - возвращается год

2.2 Убрать буквы «K», «G»… через форматирование в сетке свойств
Поставить пробелы между тысячами и добавить текст ''

(@value).toString().replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ') + ' штук'
Округлить до 0 заков после запятой, поставить пробелы и приписать "Ваш текст"

2.3 Сделать все значения на гистограмме больше нуля (код JS)
w.series = w.series.map(function(item){
...
2.4 В виджете-показателе выведите число с ошибкой до одного символа после запятой
Строку
...
Обрезанное значение, а не математическое округление!

2.5 Обрезание серийных номеров до n знаков после запятой (код JS)
w.plotOptions.series.dataLabels.formatter = function(){
    return Number((this.y).toFixed());
};
*.toFixed(0) или *.toFixed() -обрезание значения до адреса. В скобках количество знаков после запятой. Важно: значение просто обрезается, а не округляется!

2.6 Форматирование подписных диаграмм с накоплением (код JS)
Округление
w.yAxis.stackLabels.formatter = function(){
    return Math.round(this.total).toString().replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ')
}
Было: 123456789,12345 Стало: 123 456 789

Вывод измерений в миллионахмиллиардах ()
w.yAxis.stackLabels.formatter = function(){
    return Math.round(this.total/100000)/10
}
В мире мы выводим в миллионах с точностью до одного знака после запятой. Для этого делим потом на сто тысяч, округляем, делим еще на 10.
Было: 123456789,12345 Стало: 123,4
Аналогично можно сделать вывод в миллиард и с приближением до достижения знака. Также можно добавить:
...
2.7 Работа с подписями осей
w.xAxis.categories = w.xAxis.categories.map(function(item){
    return item + " кв";
})
Товар - текущая подпись. "кв" к подписке

w.xAxis.categories = w.xAxis.categories.map(function(item){
    return item.split('-').reverse().join('.');
})
Это пример для форматирования даты в формате гггг-мм-дд. Подпись 01.01.2016 превращается в 01.01.2016

w.xAxis.categories = ['Первый', 'Второй']
В этом мы хардкодим категории
...
3. Таблицы
3.1 Наименования столбцов в таблицах. Код вставляется в TableRender
Старый вариант
...
Замена наименования столбца над данными для виджета "Таблица" При обнаружении наименования столбца, правильно "__Сведения о ЧС", код заменяет его на "Значение показателя". Если 2 камеры с наименованиями "__Сведения о ЧС" находятся рядом, то они объединяются под новым названием "Значение показателя".

w.data.columns.forEach(function (column) {
    column.captions[0] = column.captions[0].replace("__Сведения о ЧС", "Значение показателя");
})
3.2 Операции со столбцами. Код вставки после TableRender
3.2.1 Выбор столбца
Столбец выпадает и превращается в переменную:
...
td:first-child - первый столбец td:last-child – последний столбец таблицы, td:nth-last-child(2) – предпоследний столбец, td:nth-child(1) – второй по счету столбец, td:nth- child(2) – третий по счету столбец.
...
3.2.6 Скрытие последнего столбца таблицы
...
3.3 Покрасить шапку таблицы. Код вставки после TableRender
Эта настройка была реализована в таблице!!!

var headColor = '#FDCE18'
$('#table-' + w.general.renderTo + ' thead').css('background',headColor);
headColor - цвет шапки

3.4 Закрепите шапку таблицы. Код вставки после TableRender
Работает только вместе с подкраской шапки. В случае необходимости под шапкой будут помещены в спираль.
...
3.5 Покрасить таблицу в зависимости от значения
...
3.6 Покрасить числовую игру на ячейке в зависимости от значения
...
В связи с этим у нас два условия: для значений меньше нуля и больше стажа. Можно изменить цвет и добавить условия

Раскраска числовых данных определения столбцов в зависимости от значений
...
3.7 Убрать слияние строк в таблице (код JS)
Работает только с дополнительной надстройкой на сервере! DisableLeftColspan: правда
...
3.11 Сделать установку личной информации
Когда информация содержится в одном столбце, необходимо вывести в одном столбце. Все привязываем в "столбцы". Скрываем строку, скрываем "показатель".
...
3.12 Скрыть заголовок заголовка таблицы (не виджета)
Вставить после TableRender

$('#table-' + w.general.renderTo + ' thead').css({
    'display': 'none'
})
3.13 Скрыть границу таблицы
Вставить после TableRender

$('#table-' + w.general.renderTo + ' table').css({
    'border': 'none'
})
3.14 Запрет разбивания слов (td = таблица, th = заголовок)
Вставить после TableRender
...
Расчет добавлением кода c# в разделе "Аналитика". Шаги выполнения:

ДАТА

Обработка дат, если в столбце нет нулевой или пустой строки

DateTime.Parse(["15","24","DATE"]) - преобразовывает строку в формат DateTime для того, чтобы позволить использовать ее в расчетах. 
Если в столбце используется пустая строка или нуль, то необходимо подставить в другое значение (в использовании текущей даты)
...
3.17 Изменить значения ячеек жирными и закрытыми по центру
Для всей таблицы
...
При копировании кода виджета со страницы https://www.highcharts.com/ в пользовательский виджет «”container”» поменять на его надпись: w.general.renderTo
...
Добавить форму со значением (в случае применения значения =38).
...
4.7 Сгруппировать столбцы гистограммы по две. Одна будет рисоваться поверх другой.
Проверено на 2.11

Код добавить в гистограмму к конструктору
...
5.1 Перенос записи (в заголовке) на новую запись
Перед самим заголовком ставится следующая комбинация символов:

5.2 Перед заголовком виджета отступа от рамки
Перед самим заголовком ставится то, что в кавычках: « »

5.3 Перед заголовком виджета отступа от рамки (отступ меньше)
Перед самим заголовком ставится то, что в кавычках: « »

5.4 Сделать посилання
Вставить в поле для текста в PG
...
5.5 Оформить ссылку как установить
TextRender({
    text: w.general,
    style: w.style
});
...
6.3 Динамическая смена подписки в правах владения областью при дрилдауне
...
Дополнительный текстовый виджет, куда кладется код для синхронного дриллдауна:
...
В фильтре/текстовом виджете пишется выбранное на влияющем виджете
...
6.7 Накопление на виджет График-диаграмма с областями
...
6.8 Скройте колонки в графике/гистограмме (когда нажимаете F5, часть серий "выключена")
Вставляется в самое начало:
...
6.9 Добавить переход по клику значения пох дракона
...
6.10 СОРТИРОВКА НА ВТОРОМ УРОВНЕ
...
Скрипт, лежащий по адресу K:\ОСИ\Отдел управления проектами\Аналитикам\global-filters v6.zip необходимо положить на сервер, тогда во всех фильтрах с пробелом в конце названия будет звучать исключительное значение.
...
7.6 Автоматическая установка в фильтре значения на сентябрь
Ставит последнее пришедшее знание в данные
Всегда работает, даже если есть внешнее влияние
...
changePOK();
8 Плоские таблицы
Общая часть Для использования кода на плоских таблицах необходимо использовать общие корректировки стандартного кода, в дальнейшем дополняться различными вариантами кода и доделками внешнего вида. Данные куски кода являются обязательными!
...
//ЧАСТЬ 1 – Установка стиля таблицы
...
//ЧАСТЬ2 – Управление приложениями таблицы
...
dataGrid;   // Важная строка, обязательно должно быть в самом конце всего кода
8.1 Выравнивание плоской таблицы по правому краю и левому краю
Код вставляется под комментарием с номером части

// В можно указать какой столбец по номеру (начиная с 0) какому выравниванию (по правому или левому краю) соответствует.
...
Добавляется в области Часть 1 (Код вставляется под комментарием с номером части)
...
8.5 Отключение пограничной прокрутки
Код вставляется под комментарием с номером части

ЧАСТЬ 2

dataGrid.dataGridInstance.option("pager", {visible: false});// отключение постраничной прокрутки
8.6 Переход на другой лист дэшборда при встрече на конкретном элементе
...
Идентификатор листа можно получить скопировав посилання на лист. В конце будет указана переменная «sheetGuid». &sheetGuid=bd32afddbc8043d0b49651078f3a6564&fit=true
...
Для использования кода в сводных таблицах необходимо наблюдать общие корректировки стандартного кода, в дальнейшем дополняться этими блоками, различающимися фрагментами кода и доделками внешнего вида. Данные куски кода являются обязательными!
...
if(event.area === "column")
       $(event.cellElement).css("font-weight", "bold");
9.2 Задать выравнивание столбцов по праву на край
Код необходимо задавать для каждого столбца таблицы. Нумерация столбцов с 0. Код вставляется под комментарием с номером части
...
Высота задается отступом от верхней и нижней границы строки в пикселях. Код вставляется под комментарием с номером части
...
9.4 Сводная таблица, до второго уровня по умолчанию
Заменить все.
...
10.1 Форматирование легенд и подсказок
Для подписки в подсказке используйте html-символы. Пример: шт -html коды Отдельные буквы «ш» и «т»
...
11 Изображение
11.1 Изображение как кнопка (прозрачное/с крышкой)
